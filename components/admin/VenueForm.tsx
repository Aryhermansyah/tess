...
export const VenueForm: React.FC = () => {
  ...
  const handleChange = async (field: keyof Venue, value: string) => {
    // Jika field adalah image/photo (misal 'image' atau 'photo'), upload ke Supabase Storage
    if ((field === 'image' || field === 'photo') && value && !value.startsWith('http')) {
      try {
        setLoading(true);
        const fileName = `venue_${Date.now()}.jpg`;
        ...
        console.error('Gagal upload gambar venue:', err);
        Alert.alert('Upload Gagal', 'Gagal upload gambar ke Supabase');
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('admin_core')
        .upsert([{ venue: formData }], { onConflict: 'id' });
      if (error) {
        throw error;
      }
