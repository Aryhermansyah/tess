...
export const CommitteeForm: React.FC = () => {
  ...
  const handleChange = (field: keyof CommitteeMember, value: string) => {
    setCurrentMember({
      ...currentMember,
      [field]: value,
    });
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.log('Saving committee data:', JSON.stringify(members).length, 'bytes');
        ...
        Alert.alert(
          'Gagal Menyimpan',
          'Terjadi kesalahan saat menyimpan data. Coba kurangi ukuran foto atau hapus beberapa anggota.',
          [{ text: 'OK' }]
        );
      } finally {
        setLoading(false);
