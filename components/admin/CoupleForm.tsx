...
export const CoupleForm: React.FC = () => {
  ...
  const handleChange = async (field: string, subfield: string, value: string) => {
    console.log(`Mengubah ${field}.${subfield} dengan nilai baru`);
    // Jika field adalah photo, upload ke Supabase Storage
    if (subfield === 'photo' && value && !value.startsWith('http')) {
      try {
        setLoading(true);
        const fileName = `${field}_${Date.now()}.jpg`;
        const url = await uploadImageToSupabase('wedding-images', value, fileName);
        if (url) {
          // Update state dengan URL dari Supabase
          setFormData(prev => ({
            ...prev,
            [field]: {
              ...prev[field as keyof Couple],
              [subfield]: url,
            },
          }));
        } else {
    ...
    // Untuk foto, simpan juga di localStorage untuk web
    if (subfield === 'photo') {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('admin_core')
        .upsert([{ couple: formData }], { onConflict: 'id' });
      if (error) {
        throw error;
      }
