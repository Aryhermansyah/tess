...
export const CoordinatorsForm: React.FC = () => {
  ...
  const handleCoordinatorChange = async (field: keyof Coordinator, value: string) => {
    if (field === 'photo' && value && !value.startsWith('http')) {
      try {
        setLoading(true);
        const fileName = `coordinator_${Date.now()}.jpg`;
        ...
        console.error('Gagal upload gambar coordinator:', err);
        Alert.alert('Upload Gagal', 'Gagal upload gambar ke Supabase');
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await updateCoordinators(coordinatorsList);
      if (error) {
        throw error;
      }
      setSuccess(true);
