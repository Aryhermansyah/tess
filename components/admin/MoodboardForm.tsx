...
export const MoodboardForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('admin_core')
        .upsert([{ moodboard: moodboardItems }], { onConflict: 'id' });
      if (error) {
        throw error;
      }
  ...
  const handleImageSelected = async (value: string) => {
    if (!value.startsWith('http')) {
      try {
        setLoading(true);
        const fileName = `moodboard_${Date.now()}.jpg`;
        ...
        console.error('Gagal upload gambar moodboard:', err);
        Alert.alert('Upload Gagal', 'Gagal upload gambar ke Supabase');
      } finally {
        setLoading(false);
      }
